...
df_B = pd.DataFrame(SERIES_B_RAW, columns=["День", "CTR"])
df_B["День"] = pd.to_datetime(df_B["День"])
df_B["CTR"] = df_B["CTR"].apply(_pct_str_to_float)
df_B = df_B.dropna().sort_values("День").reset_index(drop=True)

# --- Новая серия (оранжевая линия, проценты -> доли) ---
SERIES_ORANGE_RAW = [
    ("2025-04-22","0,52 %"),
    ("2025-04-23","0,48 %"),
    ("2025-04-24","0,58 %"),
    ...
    ("2025-07-21","0,38 %"),
    ("2025-07-22","0,37 %"),
    # если есть продолжение после 22 июля – просто допиши сюда по тому же формату
]

df_ORANGE = pd.DataFrame(SERIES_ORANGE_RAW, columns=["День", "CTR"])
df_ORANGE["День"] = pd.to_datetime(df_ORANGE["День"])
df_ORANGE["CTR"] = df_ORANGE["CTR"].apply(_pct_str_to_float)
df_ORANGE = df_ORANGE.dropna().sort_values("День").reset_index(drop=True)
...
EXTRA_SERIES = [
    ...
    },
    {
        ...
        "style": {
            "dash": "solid",
            "width": 2.4,
            "color": "rgba(255,140,0,1)",  # ярко-оранжевый
            "marker_size": 5,
        },
    },
]
...
if page == "Итоги":
    ...
    st.markdown("### Итоги: локальные пики CTR и факторы (окно ±10 дней, робастный критерий)")

    # --- Границы флайтов (для «Номер креатива») ---
    ...
    def month_index_from_anchor(d: pd.Timestamp, anchor=pd.Timestamp("2025-04-22")) -> int:
        # «Начиная с 22 апреля»: смещаем якорь на 21 день, чтобы 22-е попадало в первый месяц
    ...
    def creative_change_within_plus5(d: pd.Timestamp) -> bool:
        # Смена креатива считается если в интервале [день; день+5] есть переключение
        return any(d <= cp <= d + pd.Timedelta(days=5) for cp in (b2, b3, b4))

    def event_exact_titles(d: pd.Timestamp) -> list[str]:
        # Событие фиксируется, если начало или окончание совпало с датой
        if df_events is None or df_events.empty: return []
        rows = df_events[
    ...
    # Подхватываем «Общий трафик» если он объявлен (Н / В / О)
    if "df_N" in globals(): base = base.merge(df_N, on="День", how="left")   # колонка "N"
    if "df_V" in globals(): base = base.merge(df_V, on="День", how="left")   # колонка "V"
    if "df_O" in globals(): base = base.merge(df_O, on="День", how="left")   # колонка "O"
    ...
    # окно локальности
    W = 10
    # минимальное z относительно медианы окна
    z_min = 1.0
    # относительные приращения к среднему окна без центра:
    relA0, relB0 = 0.04, 0.03
    target_low, target_high = 20, 25  # хотим ~20–25 пиков
    step, max_iters = 0.005, 30

    # Предрасчёт по флайтам (для минимального порога на уровне флайта)
    ...
    def make_candidates(relA, relB):
        ...
        for i in range(len(base)):
            ...
            med, sigma = np.nanmedian(win), 1.4826 * mad(win.values)
            ...
            # мягкая отсечка по флайту
            d = base["День"].iloc[i]
            cr = creative_number_for_day(d)
            if cr and not (np.isnan(flight_meds[cr]) or np.isnan(flight_sigs[cr])):
                if c < flight_meds[cr] + 0.5 * flight_sigs[cr]:
                    continue
    ...
    def nms_keep_max(idx, gap=2):
        # Оставляем максимум внутри близких индексов (некое NMS по оси X)
        if not idx: return []
        idx = sorted(set(idx))
        kept, cluster = [], [idx[0]]
        for a, b in zip(idx, idx[1:]):
            if b - a <= gap:
                ...
                kept.append(max(cluster, key=lambda j: base["CTR"].iloc[j]))
                cluster = [b]
        kept.append(max(cluster, key=lambda j: base["CTR"].iloc[j]))
        return kept

    relA, relB = relA0, relB0
    best_pack = ([], 1e9)  # (indices, score)
    ...
    for _ in range(max_iters):
        ...
        if score < best_pack[1]:
            best_pack = (picked, score)
        if target_low <= cnt <= target_high:
            final_indices = picked
            break
        # Подкручиваем порог, чтобы приблизиться к таргету
        if cnt < target_low:
            relA -= step; relB -= step
        else:
            relA += step; relB += step
    else:
        final_indices = best_pack[0]

    # ---------- Формирование таблицы пиков ----------
    def _local_above_mean(i: int, col: str) -> bool:
        if col not in base.columns: return False
        s = base[col]
        L, R = max(0, i - W), min(len(s) - 1, i + W)
        win = s.iloc[L:R+1].copy()
        if base.index[i] in win.index:
            win = win.drop(base.index[i], errors="ignore")
        return bool(len(win) and pd.notna(s.iloc[i]) and (s.iloc[i] > win.mean()))
    ...
    for i in final_indices:
        ...
        peaks_rows.append({
            ...
            "Локальные просмотры баннеров выше – да/нет": "да" if _local_above_mean(i, "V") else "нет",
            "Локально больше Н-пользователей – да/нет": "да" if _local_above_mean(i, "N") else "нет",
            "Локально больше О-пользователей – да/нет": "да" if _local_above_mean(i, "O") else "нет",
    ...
    df_peaks["Дата"] = df_peaks["Локальные пики CTR (±10 дней) – дата"].dt.strftime("%d.%m.%Y")
    df_peaks["CTR (в %)"] = df_peaks["_CTR"].map(lambda x: f"{x:.2%}" if pd.notna(x) else "")

    # ---------- Таблица пиков (отображение) ----------
    cols_order = [
        "Дата",
        "CTR (в %)",
    ...
    st.markdown("#### Таблица локальных пиков CTR")
        ...
        with cols_b[i % 5]:
            st.markdown(
                f"""<div class="kpi-card">
                        <div class="kpi-title">Совпало {k} факторов</div>
    ...
    def _only_factor_name(row) -> str:
        ...
        return ""
    df_one = df_peaks.loc[df_peaks["_factors_yes"] == 1, ["Локальные пики CTR (±10 дней) – дата", "_CTR"] + factor_cols].copy()
    ...
    st.dataframe(df_one[["Дата", "Единственный фактор", "CTR (в %)"]].sort_values("Дата"),
                 use_container_width=True, hide_index=True)

    # ---------- Экспорт (CSV и XLSX) ----------
    st.markdown("#### Экспорт")
    # Готовим «красивую» таблицу к выгрузке (без служебных колонок)
    export_cols = cols_order + []  # сохраняем тот же набор, что и на экране
    df_export = df_peaks[export_cols].copy()
    ...
    with pd.ExcelWriter(xlsx_buf, engine="xlsxwriter") as writer:
        df_export.to_excel(writer, sheet_name="Peaks", index=False)
        df_one[["Дата", "Единственный фактор", "CTR (в %)"]].to_excel(writer, sheet_name="OnlyOneFactor", index=False)
    st.download_button(
        "⬇️ Скачать пики (XLSX)",
        data=xlsx_buf.getvalue(),
        file_name="peaks_ctr.xlsx",
